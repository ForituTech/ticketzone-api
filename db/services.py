...
from typing import Optional, Type, TypeVar, Generic, Protocol, Any, List, Union
...
class UpdateService(Generic[ModelType, UpdateSerializer]):
    ...
    def update(self: Union[Any, ServiceInterface], *, obj: ModelType, obj_in: UpdateSerializer) -> ModelType:
        if hasattr(self, "on_pre_update"):
            self.on_pre_update(obj_in)
        obj = self.model.objects.filter(pk=obj.id).update(**obj_in.validated_data)
        if hasattr(self, "on_post_update"):
            self.on_post_update(obj_in)
...
class ReadService(Generic[ModelType]):
    ...
    def get_filtered(self: Union[Any, ModelType], *, filters: Optional[dict[str, Any]] = None, limit: Optional[int] = None) -> List[ModelType]:
        if filters:
